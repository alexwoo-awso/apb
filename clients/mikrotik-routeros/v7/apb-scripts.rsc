...
/system script add name="apb-sync" policy=read,write,test dont-require-permissions=no comment="APB: applies incremental blocklist changes" source="# APB incremental sync for edge-1.\r\n# Applies the changes the server has queued since the curs\
    ...
    (\$K = \"+\") do={ :if ([:len [/ip firewall address-list find list=\$List address=\$V]] = 0)\
     do={ :do { /ip firewall address-list add list=\$List address=\$V timeout=\$Timeout comment=\
    \"apb\" ; :set Added (\$Added + 1) } on-error={} } }\r\n:if (\$K = \"-\") do={ :do { /ip fir\
    ewall address-list remove [find list=\$List address=\$V] ; :set Removed (\$Removed + 1) } on\
...
/system script add name="apb-bootstrap" policy=read,write,test dont-require-permissions=no comment="APB: rebuilds the whole list after a reboot" source="# APB full rebuild for edge-1.\r\n# The blocklist lives in RAM, so it is empty after a reboot. T\
    ...
    \r\n:foreach Tok in=[:toarray (\$Res->\"data\")] do={\r\n:if ([:len \$Tok] > 1) do={\r\n:loc\
    al K [:pick \$Tok 0 1]\r\n:local V [:pick \$Tok 1 [:len \$Tok]]\r\n:if (\$K = \"+\") do={ :d\
    o { /ip firewall address-list add list=\$List address=\$V timeout=\$Timeout comment=\"apb\" \
    ; :set Total (\$Total + 1) } on-error={} }\r\n:if (\$K = \"c\") do={ :set NewCursor [:tonum \
    \$V] }\r\n:if (\$K = \"n\") do={ :set Next [:tonum \$V] }\r\n}\r\n}\r\n} else={ :set Failed \
    true }\r\n} else={ :set Failed true }\r\n}\r\n} while=((!\$Failed) && (\$Next > 0) && (\$Pag\
    e < \$MaxPages))\r\n:if (\$Failed) do={\r\n:log error \"APB: rebuild failed, the sync script\
     will retry\"\r\n} else={\r\n:if (\$NewCursor >= 0) do={ :set apbCursor \$NewCursor }\r\n:lo\
    g info (\"APB: rebuild complete, \" . \$Total . \" addresses, cursor \" . \$NewCursor)\r\n}\
