...
:set apbSrc ($apbSrc . "List address=\$V timeout=8w comment=\"apb\" ; :set Added (\$Added + 1) } on-error={} } }\r\n")
...
:set apbSrc ($apbSrc . ":if (\$K = \"+\") do={ :set Recv (\$Recv + 1) ; :do { /ip firewall address-list add list=\$List address=\$V timeout=8w comment=\"apb\" ; :set Total (\$Total + 1) } on-err")
