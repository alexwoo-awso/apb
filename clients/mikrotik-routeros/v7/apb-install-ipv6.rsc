...
:set apbSrc ($apbSrc . "cate=yes-without-crl output=user as-value] } on-error={ :log warning (\"APB: sync could not reach \" . \$Url . \" - run apb-test for detail\") }\r\n")
...
:set apbSrc ($apbSrc . ":if (\$K = \"+\") do={ :set Recv (\$Recv + 1) ; :if ([:typeof [:find \$V \":\"]] = \"num\") do={ :if ([:len [/ipv6 firewall address-list find list=\$List address=\$V]] = ")
:set apbSrc ($apbSrc . "0) do={ :do { /ipv6 firewall address-list add list=\$List address=\$V timeout=8w comment=\"apb\" ; :set Added (\$Added + 1) } on-error={} } } else={ :if ([:len [/ip firew")
:set apbSrc ($apbSrc . "all address-list find list=\$List address=\$V]] = 0) do={ :do { /ip firewall address-list add list=\$List address=\$V timeout=8w comment=\"apb\" ; :set Added (\$Added + 1")
:set apbSrc ($apbSrc . ") } on-error={} } } }\r\n")
:set apbSrc ($apbSrc . ":if (\$K = \"-\") do={ :if ([:typeof [:find \$V \":\"]] = \"num\") do={ :do { /ipv6 firewall address-list remove [find list=\$List address=\$V] ; :set Removed (\$Removed ")
:set apbSrc ($apbSrc . "+ 1) } on-error={} } else={ :do { /ip firewall address-list remove [find list=\$List address=\$V] ; :set Removed (\$Removed + 1) } on-error={} } }\r\n")
...
:set apbSrc ($apbSrc . "st=\$State address=192.0.2.1 comment=(\"\" . \$apbCursor) timeout=8w } on-error={ :log warning (\"APB: could not record cursor \" . \$apbCursor . \" in \" . \$State . \" ")
:set apbSrc ($apbSrc . "- the router refused the address-list write\") } } }\r\n")
:set apbSrc ($apbSrc . ":if ((\$Recv > 0) && (\$Added = 0)) do={ :log error (\"APB: the server sent \" . \$Recv . \" addresses and this router stored none of them. It is refusing timeout=8w on a")
...
:set apbSrc ($apbSrc . ":if (\$K = \"+\") do={ :set Recv (\$Recv + 1) ; :if ([:typeof [:find \$V \":\"]] = \"num\") do={ :do { /ipv6 firewall address-list add list=\$List address=\$V timeout=8w ")
:set apbSrc ($apbSrc . "comment=\"apb\" ; :set Total (\$Total + 1) } on-error={} } else={ :do { /ip firewall address-list add list=\$List address=\$V timeout=8w comment=\"apb\" ; :set Total (\$T")
:set apbSrc ($apbSrc . "otal + 1) } on-error={} } }\r\n:if (\$K = \"c\") do={ :set NewCursor [:tonum \$V] }\r\n:if (\$K = \"n\") do={ :set Next [:tonum \$V] }\r\n}\r\n}\r\n")
...
:set apbSrc ($apbSrc . "0.2.1 comment=(\"\" . \$apbCursor) timeout=8w } on-error={ :log warning (\"APB: could not record cursor \" . \$apbCursor . \" in \" . \$State . \" - the router refused th")
:set apbSrc ($apbSrc . "e address-list write\") } }\r\n")
:set apbSrc ($apbSrc . ":if ((\$Recv > 0) && (\$Total = 0)) do={ :log error (\"APB: the server sent \" . \$Recv . \" addresses and this router stored none of them. It is refusing timeout=8w on a")
...
:set apbSrc ($apbSrc . ":if ([:typeof \$apbReportLock] = \"time\") do={ :if ((\$Now - \$apbReportLock) < 10m) do={ :set Run false } }\r\n:if (\$Run) do={\r\n:set apbReportLock \$Now\r\n:do {\r\n")
:set apbSrc ($apbSrc . ":local Payload \"\"\r\n:local Count 0\r\n:foreach Id in=[/ip firewall address-list find list=\$Detect] do={\r\n:if (\$Count < \$MaxBatch) do={\r\n:local A \"\"\r\n")
:set apbSrc ($apbSrc . ":do { :set A [/ip firewall address-list get \$Id address] } on-error={}\r\n:if ([:len \$A] > 0) do={\r\n")
...
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=(\$Url . \"/report\?k=\" . \$UA) http-method=post http-data=\$Payload http-header-field=(\$Hdr . \$Meta) mode=https check-certificate=yes-")
:set apbSrc ($apbSrc . "without-crl output=user as-value] } on-error={ :log warning (\"APB: report could not reach \" . \$Url . \" - run apb-test for detail\") }\r\n")
:set apbSrc ($apbSrc . ":if ([:typeof \$Res] = \"array\") do={\r\n:if ((\$Res->\"status\") = \"finished\") do={\r\n:local Data (\$Res->\"data\")\r\n:if ([:pick \$Data 0 2] = \"ok\") do={\r\n")
:set apbSrc ($apbSrc . ":foreach A in=[:toarray \$Payload] do={ :do { /ip firewall address-list add list=\$Sent address=\$A timeout=1d comment=\"apb-sent\" } on-error={} }\r\n")
...
:set apbSrc ($apbSrc . ":local Timeout 8w\r\n:local Auth (\"Authorization: Bearer \" . \$Token)\r\n:local Hdr (\"Authorization: Bearer \" . \$Token . \",User-Agent: \" . \$UA)\r\n")
:set apbSrc ($apbSrc . ":local Target (\$Url . \"/whoami\?k=\" . \$UA)\r\n:local Res \"\"\r\n\r\n")
:set apbSrc ($apbSrc . ":log info (\"APB test: calling \" . \$Target . \" as https, certificate check yes-without-crl\")\r\n")
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=\$Target http-method=get http-header-field=\$Hdr mode=https check-certificate=yes-without-crl output=user as-value] } on-error={ :log erro")
:set apbSrc ($apbSrc . "r \"APB test: the real request failed. Causes in order of likelihood: the name does not resolve, there is no route to the server, or the certificate is rejected.\" }\r\n")
:set apbSrc ($apbSrc . ":if ([:typeof \$Res] = \"array\") do={\r\n:log info (\"APB test: status \" . (\$Res->\"status\"))\r\n:log info (\"APB test: reply \" . (\$Res->\"data\"))\r\n")
:set apbSrc ($apbSrc . ":if ((\$Res->\"status\") = \"finished\") do={\r\n")
:set apbSrc ($apbSrc . ":if ([:pick (\$Res->\"data\") 0 2] = \"1,\") do={ :log info \"APB test: PASS - the server answered with this device's configuration, so the address, the token and the ide")
:set apbSrc ($apbSrc . "ntity are all correct\" } else={ :log error \"APB test: the server replied but not with a configuration line, so the token is wrong or revoked\" }\r\n}\r\n} else={\r\n")
:set apbSrc ($apbSrc . ":log error \"APB test: nothing came back at all\"\r\n}\r\n\r\n# Header shape probe. Each variant is the same request with a different header\r\n")
:set apbSrc ($apbSrc . "# set; a failure here means RouterOS refused to build the request, not that the\r\n# server said no.\r\n:local Probe \"\"\r\n")
:set apbSrc ($apbSrc . ":do { :set Probe [/tool fetch url=\$Target http-method=get http-header-field=\$Auth mode=https check-certificate=yes-without-crl output=user as-value] ; :log info (\"APB ")
:set apbSrc ($apbSrc . "test: probe A, authorization only: \" . (\$Probe->\"status\")) } on-error={ :log warning \"APB test: probe A, authorization only: REFUSED by RouterOS\" }\r\n")
:set apbSrc ($apbSrc . ":do { :set Probe [/tool fetch url=\$Target http-method=get http-header-field=(\$Auth . \",User-Agent: \" . \$UA) mode=https check-certificate=yes-without-crl output=user ")
:set apbSrc ($apbSrc . "as-value] ; :log info (\"APB test: probe B, capitalised User-Agent: \" . (\$Probe->\"status\")) } on-error={ :log warning \"APB test: probe B, capitalised User-Agent: REF")
:set apbSrc ($apbSrc . "USED by RouterOS\" }\r\n")
:set apbSrc ($apbSrc . ":do { :set Probe [/tool fetch url=\$Target http-method=get http-header-field=(\$Auth . \",user-agent: \" . \$UA) mode=https check-certificate=yes-without-crl output=user ")
:set apbSrc ($apbSrc . "as-value] ; :log info (\"APB test: probe C, lowercase user-agent: \" . (\$Probe->\"status\")) } on-error={ :log warning \"APB test: probe C, lowercase user-agent: REFUSED")
:set apbSrc ($apbSrc . " by RouterOS\" }\r\n")
:set apbSrc ($apbSrc . ":do { :set Probe [/tool fetch url=\$Target http-method=get http-header-field=(\$Auth . \",X-Apb-Agent: \" . \$UA) mode=https check-certificate=yes-without-crl output=user")
:set apbSrc ($apbSrc . " as-value] ; :log info (\"APB test: probe D, custom X- header: \" . (\$Probe->\"status\")) } on-error={ :log warning \"APB test: probe D, custom X- header: REFUSED by Rou")
:set apbSrc ($apbSrc . "terOS\" }\r\n:log info \"APB test: done with headers. The identity travels in the URL, so a REFUSED probe costs nothing; it only records what this build tolerates.\"\r\n")
...
:set apbSrc ($apbSrc . "# the router's flash. If that is refused, nothing else matters. These vary one\r\n# thing at a time - the address, the size of the timeout, and whether it is\r\n")
:set apbSrc ($apbSrc . "# written literally or passed through a variable - so the refusal can be\r\n# attributed rather than guessed at.\r\n:local Timeout 8w\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list add list=\$State address=1.2.3.4 timeout=1d ; :log info \"APB test: write J, ordinary address, 1d: OK\" } on-error={ :log error \"APB test")
:set apbSrc ($apbSrc . ": write J, ordinary address, 1d: REFUSED\" }\r\n:do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list add list=\$State address=192.0.2.1 timeout=1d ; :log info \"APB test: write K, marker address 192.0.2.1, 1d: OK\" } on-error={ :log error ")
...
:set apbSrc ($apbSrc . "EFUSED\" }\r\n:do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list add list=\$State address=1.2.3.4 timeout=12w ; :log info \"APB test: write M4, 12w: OK\" } on-error={ :log error \"APB test: write M4, 12w")
:set apbSrc ($apbSrc . ": REFUSED\" }\r\n:do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list add list=\$State address=1.2.3.4 timeout=16w ; :log info \"APB test: write M5, 16w: OK\" } on-error={ :log error \"APB test: write M5, 16w")
:set apbSrc ($apbSrc . ": REFUSED\" }\r\n:do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list add list=\$State address=1.2.3.4 timeout=24w ; :log info \"APB test: write M6, 24w: OK\" } on-error={ :log error \"APB test: write M6, 24w")
:set apbSrc ($apbSrc . ": REFUSED\" }\r\n:do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
...
:set apbSrc ($apbSrc . "test: write N, 520w written literally: REFUSED\" }\r\n:do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list add list=\$State address=1.2.3.4 timeout=\$Timeout ; :log info \"APB test: write O, the configured timeout through a variable: OK\" } on-e")
:set apbSrc ($apbSrc . "rror={ :log error \"APB test: write O, the configured timeout through a variable: REFUSED\" }\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list add list=\$State address=1.2.3.4 timeout=8w comment=\"probe\" ; :log info \"APB test: write P, exactly what the real scripts do: OK\" } on")
:set apbSrc ($apbSrc . "-error={ :log error \"APB test: write P, exactly what the real scripts do: REFUSED\" }\r\n:do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
:set apbSrc ($apbSrc . ":log info \"APB test: finished. The largest timeout that says OK is the one to set as the entry timeout on the device page. Every probe carries a timeout, so none of this")
:set apbSrc ($apbSrc . " reached the router's flash.\"\r\n")
