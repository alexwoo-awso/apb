...
:set apbSrc ($apbSrc . "all address-list add list=\$List address=\$V timeout=4w comment=\"apb\" ; :set Added (\$Added + 1) } on-error={} } } else={ :if ([:len [/ip firewall address-list find lis")
:set apbSrc ($apbSrc . "t=\$List address=\$V]] = 0) do={ :do { /ip firewall address-list add list=\$List address=\$V timeout=4w comment=\"apb\" ; :set Added (\$Added + 1) } on-error={} } } }\r\n")
...
:set apbSrc ($apbSrc . ":if (\$K = \"+\") do={ :if ([:typeof [:find \$V \":\"]] = \"num\") do={ :do { /ipv6 firewall address-list add list=\$List address=\$V timeout=4w comment=\"apb\" ; :set To")
:set apbSrc ($apbSrc . "tal (\$Total + 1) } on-error={} } else={ :do { /ip firewall address-list add list=\$List address=\$V timeout=4w comment=\"apb\" ; :set Total (\$Total + 1) } on-error={} }")
...
:set apbSrc ($apbSrc . ":local Timeout 4w\r\n:local Auth (\"Authorization: Bearer \" . \$Token)\r\n:local Hdr (\"Authorization: Bearer \" . \$Token . \",User-Agent: \" . \$UA)\r\n")
...
:set apbSrc ($apbSrc . ":do { /ip firewall address-list remove [find list=\$State] } on-error={}\r\n")
:set apbSrc ($apbSrc . ":do { /ip firewall address-list add list=\$State address=1.2.3.4 timeout=4w comment=\"probe\" ; :log info \"APB test: write P, exactly what the real scripts do: OK\" } on")
