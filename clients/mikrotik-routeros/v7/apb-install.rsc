...
:set apbSrc ($apbSrc . "520w comment=\"apb\" ; :set Added (\$Added + 1) } on-error={} } }\r\n")
...
:set apbSrc ($apbSrc . ":foreach Tok in=[:toarray (\$Res->\"data\")] do={\r\n:if ([:len \$Tok] > 1) do={\r\n:local K [:pick \$Tok 0 1]\r\n:local V [:pick \$Tok 1 [:len \$Tok]]\r\n")
:set apbSrc ($apbSrc . ":if (\$K = \"+\") do={ :do { /ip firewall address-list add list=\$List address=\$V timeout=520w comment=\"apb\" ; :set Total (\$Total + 1) } on-error={} }\r\n")
