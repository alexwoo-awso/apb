...
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=(\$Url . \"/sync\?c=\" . \$apbCursor . \"&n=\" . \$Held) http-method=get http-header-field=\$Hdr check-certificate=no output=user as-value")
:set apbSrc ($apbSrc . "] } on-error={ :log warning \"APB: sync request failed\" }\r\n:if ([:typeof \$Res] = \"array\") do={\r\n:if ((\$Res->\"status\") = \"finished\") do={\r\n")
...
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=\$U http-method=get http-header-field=\$Hdr check-certificate=no output=user as-value] } on-error={ :set Failed true }\r\n")
...
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=(\$Url . \"/report\") http-method=post http-data=\$Payload http-header-field=(\$Hdr . \$Meta) check-certificate=no output=user as-value] }")
:set apbSrc ($apbSrc . " on-error={ :log warning \"APB: report request failed\" }\r\n:if ([:typeof \$Res] = \"array\") do={\r\n:if ((\$Res->\"status\") = \"finished\") do={\r\n")
