...
/system script add name="apb-sync" policy=read,write,test dont-require-permissions=no comment="APB: applies incremental blocklist changes" source="# APB incremental sync for edge-1.\r\n# Applies the changes the server has queued since the curs\
    ...
    er-field=\$Hdr check-certificate=no output=user as-value] } on-error={ :log warning \"APB: s\
    ync request failed\" }\r\n:if ([:typeof \$Res] = \"array\") do={\r\n:if ((\$Res->\"status\")\
     = \"finished\") do={\r\n:local Next -1\r\n:local Resync false\r\n:local Added 0\r\n:local R\
    emoved 0\r\n:foreach Tok in=[:toarray (\$Res->\"data\")] do={\r\n:if ([:len \$Tok] > 1) do={\
    \r\n:local K [:pick \$Tok 0 1]\r\n:local V [:pick \$Tok 1 [:len \$Tok]]\r\n:if (\$K = \"+\")\
     do={ :if ([:len [/ip firewall address-list find list=\$List address=\$V]] = 0) do={ :do { /\
    ip firewall address-list add list=\$List address=\$V timeout=\$Timeout comment=\"apb\" ; :se\
    t Added (\$Added + 1) } on-error={} } }\r\n:if (\$K = \"-\") do={ :do { /ip firewall address\
    -list remove [find list=\$List address=\$V] ; :set Removed (\$Removed + 1) } on-error={} }\r\
    \n:if (\$K = \"c\") do={ :set Next [:tonum \$V] }\r\n:if (\$K = \"m\") do={ :set More true }\
    \r\n:if (\$K = \"r\") do={ :set Resync true }\r\n}\r\n}\r\n:if (\$Resync) do={\r\n:log info \
    \"APB: cursor too old, falling back to a full rebuild\"\r\n:set More false\r\n/system script\
     run apb-bootstrap\r\n} else={\r\n:if (\$Next >= 0) do={ :set apbCursor \$Next }\r\n:if ((\$\
    Added + \$Removed) > 0) do={ :log info (\"APB: applied +\" . \$Added . \" -\" . \$Removed . \
...
/system script add name="apb-bootstrap" policy=read,write,test dont-require-permissions=no comment="APB: rebuilds the whole list after a reboot" source="# APB full rebuild for edge-1.\r\n# The blocklist lives in RAM, so it is empty after a reboot. T\
    ...
    U http-method=get http-header-field=\$Hdr check-certificate=no output=user as-value] } on-er\
    ror={ :set Failed true }\r\n:if (!\$Failed) do={\r\n:if ([:typeof \$Res] = \"array\") do={\r\
    \n:if ((\$Res->\"status\") = \"finished\") do={\r\n:if (!\$Started) do={\r\n:set Started tru\
    e\r\n:set apbCursor\r\n/ip firewall address-list remove [find list=\$List]\r\n}\r\n:foreach \
    Tok in=[:toarray (\$Res->\"data\")] do={\r\n:if ([:len \$Tok] > 1) do={\r\n:local K [:pick \
    \$Tok 0 1]\r\n:local V [:pick \$Tok 1 [:len \$Tok]]\r\n:if (\$K = \"+\") do={ :do { /ip fire\
    wall address-list add list=\$List address=\$V timeout=\$Timeout comment=\"apb\" ; :set Total\
     (\$Total + 1) } on-error={} }\r\n:if (\$K = \"c\") do={ :set NewCursor [:tonum \$V] }\r\n:i\
    f (\$K = \"n\") do={ :set Next [:tonum \$V] }\r\n}\r\n}\r\n} else={ :set Failed true }\r\n} \
    else={ :set Failed true }\r\n}\r\n} while=((!\$Failed) && (\$Next > 0) && (\$Page < \$MaxPag\
    es))\r\n:if (\$Failed) do={\r\n:log error \"APB: rebuild failed, the sync script will retry\
    \"\r\n} else={\r\n:if (\$NewCursor >= 0) do={ :set apbCursor \$NewCursor }\r\n:log info (\"A\
    PB: rebuild complete, \" . \$Total . \" addresses, cursor \" . \$NewCursor)\r\n}\r\n} on-err\
    or={ :log error \"APB: rebuild script aborted\" }\r\n:set apbBootLock\r\n}\r\n"
/system script add name="apb-report" policy=read,write,test dont-require-permissions=no comment="APB: uploads locally detected addresses" source="# APB report for edge-1.\r\n# Uploads the addresses your own firewall rules have put into the de\
    ...
    header-field=(\$Hdr . \$Meta) check-certificate=no output=user as-value] } on-error={ :log w\
    arning \"APB: report request failed\" }\r\n:if ([:typeof \$Res] = \"array\") do={\r\n:if ((\
    \$Res->\"status\") = \"finished\") do={\r\n:local Data (\$Res->\"data\")\r\n:if ([:pick \$Da\
    ta 0 2] = \"ok\") do={\r\n:foreach A in=[:toarray \$Payload] do={ :do { /ip firewall address\
    -list add list=\$Sent address=\$A timeout=\$SentTimeout comment=\"apb-sent\" } on-error={} }\
    \r\n:log info (\"APB: reported \" . \$Count . \" addresses\")\r\n} else={ :log warning (\"AP\
    B: report rejected: \" . \$Data) }\r\n}\r\n}\r\n}\r\n} on-error={ :log error \"APB: report s\
    cript aborted\" }\r\n:set apbReportLock\r\n}\r\n"
/system script add name="apb-purge" policy=read,write,test dont-require-permissions=no comment="APB: clears every address APB manages here" source="# APB purge for edge-1.\r\n# Clears every address APB manages on this router and forgets the cur\
    ...
    :set apbLock\r\n:set apbBootLock\r\n:set apbReportLock\r\n:log info \"APB: local state clear\
    ed\"\r\n"
