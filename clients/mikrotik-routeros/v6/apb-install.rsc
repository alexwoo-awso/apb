...
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=(\$Url . \"/sync\?c=\" . \$apbCursor . \"&n=\" . \$Held) http-method=get http-header-field=\$Hdr mode=https check-certificate=no output=us")
:set apbSrc ($apbSrc . "er as-value] } on-error={ :log warning (\"APB: sync could not reach \" . \$Url . \" - run apb-test for detail\") }\r\n:if ([:typeof \$Res] = \"array\") do={\r\n")
...
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=(\$Url . \"/report\") http-method=post http-data=\$Payload http-header-field=(\$Hdr . \$Meta) mode=https check-certificate=no output=user ")
:set apbSrc ($apbSrc . "as-value] } on-error={ :log warning (\"APB: report could not reach \" . \$Url . \" - run apb-test for detail\") }\r\n:if ([:typeof \$Res] = \"array\") do={\r\n")
...
:set apbSrc ($apbSrc . ":local Url \"https://apb.example.org/api/v1\"\r\n:local Token \"apb_SAMPLETOKENONLY000000\"\r\n:local UA \"apb-router\"\r\n")
:set apbSrc ($apbSrc . ":local Hdr (\"Authorization: Bearer \" . \$Token . \",User-Agent: \" . \$UA)\r\n:local Res \"\"\r\n")
:set apbSrc ($apbSrc . ":log info (\"APB test: GET \" . \$Url . \"/whoami as https, certificate check no\")\r\n")
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=(\$Url . \"/whoami\") http-method=get http-header-field=\$Hdr mode=https check-certificate=no output=user as-value] } on-error={ :log erro")
:set apbSrc ($apbSrc . "r \"APB test: fetch raised an error. The usual causes are a name that does not resolve, no route to the server, or a certificate the router will not accept.\" }\r\n")
