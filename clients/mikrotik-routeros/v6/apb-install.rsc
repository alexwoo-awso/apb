...
:set apbSrc ($apbSrc . ":set Payload (\$Payload . \$A)\r\n:set Count (\$Count + 1)\r\n}\r\n}\r\n}\r\n}\r\n:if (\$Count > 0) do={\r\n:local Meta \",Content-Type: text/plain\"\r\n")
:set apbSrc ($apbSrc . ":local Res \"\"\r\n")
:set apbSrc ($apbSrc . ":do { :set Res [/tool fetch url=(\$Url . \"/report\?k=\" . \$UA) http-method=post http-data=\$Payload http-header-field=(\$Hdr . \$Meta) mode=https check-certificate=no o")
:set apbSrc ($apbSrc . "utput=user as-value] } on-error={ :log warning (\"APB: report could not reach \" . \$Url . \" - run apb-test for detail\") }\r\n:if ([:typeof \$Res] = \"array\") do={\r\n")
...
:set apbSrc ($apbSrc . ":do { :set Probe [/tool fetch url=\$Target http-method=get http-header-field=\$Auth mode=https check-certificate=no output=user as-value] ; :log info (\"APB test: probe A")
:set apbSrc ($apbSrc . ", authorization only: \" . (\$Probe->\"status\")) } on-error={ :log warning \"APB test: probe A, authorization only: REFUSED by RouterOS\" }\r\n")
:set apbSrc ($apbSrc . ":do { :set Probe [/tool fetch url=\$Target http-method=get http-header-field=(\$Auth . \",User-Agent: \" . \$UA) mode=https check-certificate=no output=user as-value] ; :")
:set apbSrc ($apbSrc . "log info (\"APB test: probe B, capitalised User-Agent: \" . (\$Probe->\"status\")) } on-error={ :log warning \"APB test: probe B, capitalised User-Agent: REFUSED by Route")
:set apbSrc ($apbSrc . "rOS\" }\r\n")
:set apbSrc ($apbSrc . ":do { :set Probe [/tool fetch url=\$Target http-method=get http-header-field=(\$Auth . \",user-agent: \" . \$UA) mode=https check-certificate=no output=user as-value] ; :")
:set apbSrc ($apbSrc . "log info (\"APB test: probe C, lowercase user-agent: \" . (\$Probe->\"status\")) } on-error={ :log warning \"APB test: probe C, lowercase user-agent: REFUSED by RouterOS")
:set apbSrc ($apbSrc . "\" }\r\n")
...
:set apbSrc ($apbSrc . "# Address-list write probe. Adding an entry that carries a timeout is the whole\r\n# basis of this design: a timeout is what keeps the entry in RAM instead of on\r\n")
